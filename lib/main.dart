...
class _PortfolioPageState extends State<PortfolioPage> {
  ...
  bool _dataLoaded = false; // 核心修复：数据加载完成标志位
  Timer? _timer;
  ...
  Future<void> _loadLocalData() async {
    try {
      ...
      final String? jsonStr = prefs.getString('my_portfolio_v2'); // 换个Key防止旧脏数据干扰
  ...
  Future<void> _saveLocalData() async {
    // 致命BUG修复：如果数据还没加载完，严禁保存！防止空数据覆盖本地。
  ...
  void _addNewFund(String code) {
    if (!_dataLoaded) return; // 未加载完成禁止操作
...
class _FundDetailPageState extends State<FundDetailPage> {
  ...
  Future<void> _fetchHistoryData() async {
    try {
      final url = Uri.parse("https://fundmobapi.eastmoney.com/FundMNewApi/FundMNHisNetList?product=EFund&appType=ttjj&FCODE=${widget.code}&PAGEINDEX=1&PAGESIZE=20");
